...
public class QDAAScreen extends Screen {
	...
	protected void init() {
		...
		addDrawableChild(new ButtonWidget((width/2)+1, height-20, 100, 20, Text.translatable("gui.done"), (b) -> {
			closeScreen();
		}));
		addDrawableChild(new ButtonWidget((width/2)-101, height-20, 100, 20, Text.translatable("options."+(QDAA.isConfigEnabled()?"on":"off")+".composed", "2xSSAA"), (b) -> {
			QDAA.setEnabled(!QDAA.isConfigEnabled());
			QDAA.writeConfig();
			b.setMessage(Text.translatable("options."+(QDAA.isConfigEnabled()?"on":"off")+".composed", "2xSSAA"));
		}));
		if (client.world == null) {
			...
			demoScreen.init(client, width, height);
		}
	}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		var ssWidth = fb.textureWidth;
		var ssHeight = fb.textureHeight;
		...
		for (int i = 0; i < 2; i++) {
			int x = ((width/2)*i);
			client.getItemRenderer().renderGuiItemIcon(Items.LIGHT_GRAY_GLAZED_TERRACOTTA.getDefaultStack(), x+30, 30);
			client.getItemRenderer().renderGuiItemIcon(Items.CRYING_OBSIDIAN.getDefaultStack(), x+55, 50);
			matrices.push();
		...
		if (client.isWindowFocused()) {
			...
			int rawMouseX = (int)(client.mouse.getX());
			int rawMouseY = (int)(client.mouse.getY());
			rawMouseX = MathHelper.clamp(rawMouseX, lensSize, winWidth-lensSize);
			rawMouseY = MathHelper.clamp(rawMouseY, lensSize, winHeight-lensSize);
		...
		drawCenteredText(matrices, textRenderer, Text.translatable("options.off"), width/4, 20, -1);
		drawCenteredText(matrices, textRenderer, Text.translatable("options.on"), (width*3)/4, 20, -1);
